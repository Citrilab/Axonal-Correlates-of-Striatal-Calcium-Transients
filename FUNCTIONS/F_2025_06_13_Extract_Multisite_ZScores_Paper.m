function [single_trial_data, trial_corr_struct] = F_2025_06_13_Extract_Multisite_ZScores_Paper(single_trial_folder, single_trial_data, exp_props, filt_opt, raw_pks_plot, pks_or_sigisos, plot_dff_zscr, site_col_names, sites_to_plot, corr_baseline_site, corr_cross, color, fig_output_folder, trial_corr_struct, corr_field_names, plt_timeline_struct, tiled_overlay, i_list)
% 2021_12_23 Extract traces from excel files:

% 0. Properties for extracting information about raw signal from each
% site:
    SEC_SMOOTH = 15;    % Calculating a smoothened signal: substract the 15-sec moving average.

% 0.1 Get/extract properties from exp_prop
    ...
    % Get num_tiles from sites_to_plot 
    num_tiles = length(sites_to_plot);
    ...
    % DavidCode properties
    LED_STATE_SIG = 2;
    LED_STATE_ISOS = 1;
%     FIT_DEGREE = 3;
    POLYFIT_N_VAL = 1;
%     SEC_TO_SUBTRACT = PHOTOM_SEC_START_SUBTRACT;
    SEC_TO_SUBTRACT = 30;
    % frames_to_subtract = PHOTOM_FR * SEC_TO_SUBTRACT;
    % 0.1 SMOOTH_FACT and other properties:
    SMOOTH_MIN = 1;
    % smooth_lowpass_window = SMOOTH_MIN * 60 * PHOTOM_FR;
    COL_TIMESTAMP = 2;

% 1. Enter zscore directory, and then get and define paths to excel
% directories:
    % 
    zscr_excel_folder = single_trial_folder;
    cd(zscr_excel_folder);
    excel_dir = dir('*.csv');               % All excel files in directory - including processed Z-score files
%     signal_file_dir = dir('*L47020*');      % Signal files in directory
%     control_file_dir = dir('*L41520*');     % Control files (Isosbestic)
...
%     signal_raw_only_allcols = readmatrix(signal_file_dir.name);     % Get matrix of all 470_only values
%     isos_raw_only_allcols = readmatrix(control_file_dir.name);      % Get matrix of all 415_only values

% 2. Get photometry frame rate from interleaved file:
    % Load the Excel file
    ...
    % Compute the difference between the last and first timestamp
...
% Define arrays to store z-score information for one trial
    for i = 1:1
        z_scr_length_min = 0;
        z_scores_allsites = [];
        dff_allsites = [];
    ...
    % 2. Loop over all sites:
    for i_site = 1:num_sites
            zscr_string = site_names{i_site};
        
        % 3A. Now, read the signal_control matrix from file name, and (optionally)
        % plot the z_score along with fitted control:

        % 3B. Now, read the raw signals
            signal_control = interleaved_matrix;
            signal_raw_site = signal_raw_only_allcols(:, cols_to_process(i_site))';
            signal_smooth_site = smooth(signal_raw_site, exp_props.pho.PHOTOM_FR)' - smooth(signal_raw_site, (SEC_SMOOTH * exp_props.pho.PHOTOM_FR))';
            isos_raw_site = isos_raw_only_allcols(:, cols_to_process(i_site))';
            isos_smooth_site = smooth(isos_raw_site, exp_props.pho.PHOTOM_FR)' - smooth(isos_raw_site, (SEC_SMOOTH * exp_props.pho.PHOTOM_FR))';
        ...
        % Now, run PMAT code to get z-scores for both signal and control at
        % this site!!!
        
        % i. Smooth signal and process and plot:
            signal_cut = signal_raw_site(frames_to_subtract:end);
            isos_cut = isos_raw_site(frames_to_subtract:end);
            signal_smooth_cut = signal_smooth_site(frames_to_subtract:end);
            isos_smooth_cut = isos_smooth_site(frames_to_subtract:end);

        if filt_opt == 1
            % A. From PMAT - process data:

%           MATLAB Description of SPAN parameter in smooth function:  Number of data points for calculating the smoothed value,
%           specified as an integer or as a scalar value in the range (0,1) denoting a fraction of the total number of data points.
%           If you use the moving average or Savitzky-Golay methods, the number of data points for calculating 
%           the smoothed value must be an odd integer. If you specify span as an even number or as a fraction 
%           that results in an even number of data points, span is automatically reduced by 1.
%           
%           DL notes on span: when specified as a fraction, the value is a
%           function of length of the recording (above)
...
%             Fsignal=smooth(signal_cut, span, 'sgolay', 3); 
%             Fisos=smooth(isos_cut, span, 'sgolay', 3);
            if length(Fsignal) > length(Fisos)
                Fsignal = Fsignal(1:end-1);
            end
%             min_trace_length = min(length(Fisos), length(Fsignal));
%             Fisos_len_eq = Fisos(1:min_trace_length);
%             Fsignal_len_eq = Fsignal(1:min_trace_length);
        ...
        elseif filt_opt == 2
            % A. Curve-fit/GUPPY - process data:
        ...
        for i = 1:1
            z_scr_lengths = [size(z_scores, 2), size(z_timeline, 2), size(control_signal, 2), size(signal_smooth_site, 2), size(isos_smooth_site, 2)];
            z_scr_length_min = min(z_scr_lengths);
%             if i_site > 1
%                 z_scores_allsites = z_scores_allsites(:, 1:z_scr_length_min);
%                 z_timelines_allsites = z_timelines_allsites(:, 1:z_scr_length_min);
%                 z_controls_allsites = z_controls_allsites(:, 1:z_scr_length_min);
%                 signal_smooth_allsites = signal_smooth_allsites(1, 1:z_scr_length_min); 
%                 isos_smooth_allsites = isos_smooth_allsites(1, 1:z_scr_length_min);
%             end
        end
        ...
        signal_smooth_allsites(i_site, :) = signal_smooth_cut(1, 1:z_scr_length_min);
        isos_smooth_allsites(i_site, :) = isos_smooth_cut(1, 1:z_scr_length_min);
%             z_signal_allsites(i_site, :) =  z_signal(1, 1:z_scr_length_min);
%             z_isos_allsites(i_site, :) =  z_isos(1, 1:z_scr_length_min);
...
%                 sublegend;
    ...
    single_trial_data.signal_control = signal_control;
